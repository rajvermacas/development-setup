...
def backup_transcript(transcript_path, trigger):
    ...
    try:
        ...
        backup_dir = Path(".dev-resources") / "context" / "compact"
...
def main():
    try:
        ...
        # Decision block: Block auto compaction, allow manual compaction
        if trigger == "auto":
            # Block auto compaction and provide feedback to Claude
            reason = "Auto-compaction blocked by precompact hook. Use manual compaction if needed."
            if backup_path:
                reason += f" Transcript backed up to: {backup_path}"
        ...
        # Provide feedback based on trigger type (manual only now)
        if args.verbose:
        ...
        # Success - compaction will proceed (manual only)
        sys.exit(0)
